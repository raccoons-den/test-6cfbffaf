    ...
    public class ResourceFetcher : IResourceFetcher
    {
        ...
        private async Task<string> ReadResource(Uri endpoint, string method, Dictionary<string, string> headers)
        {
            try
            {
                ...
                {
                    throw new Exception("The requested data is not found at " + endpoint.AbsolutePath);
                }

                throw new Exception("Failed to get resource. Error code: " + httpResponse.StatusCode +
            ...
            catch (Exception e)
            {
                ...
                throw new Exception("Failed to connect to service endpoint: ", e);
            }
        }
